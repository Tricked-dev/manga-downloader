...
Original writes stream assets into a temporary file, seal and sync it, then publish it atomically. A failed or canceled write leaves an existing container intact. Appending an upscale to a sealed container depends on the upstream append API; rebuilding a container is not a substitute for that operation.
