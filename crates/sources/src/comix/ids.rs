...
use crate::comix::http::get_json_body;
use crate::comix::parse::parse_manga_details_response;
use crate::comix::{API_URL, BASE_URL};
use base64::{
    Engine,
...
pub async fn candidate_manga_ids(
    ...
    for numeric_id in numeric_ids {
        if let Some(hash_id) = resolve_hash_id(http, &numeric_id).await
...
async fn resolve_hash_id(http: &SourceHttpClient, numeric_id: &str) -> Option<String> {
    let url = manga_details_url(numeric_id);
    let json_str = get_json_body(http, &url).await.ok()?;
