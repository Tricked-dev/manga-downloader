...
Chapter jobs run through apalis with one consumer. Completion of original downloads enqueues a job when both `auto_upscale` and `source.<key>.auto_upscale` permit it (both default to true). Default jobs use 2×. Chapters whose median original page width is at least 2000 pixels skip inference; all others retain the model's full output dimensions. `POST /v1/downloads/{id}/upscale` requests a rerun. The job stages one page output at a time and publishes all pages together with the original footer hash as a concurrency guard. Missing manifests, matching model bands or model files finish the job without retries.
