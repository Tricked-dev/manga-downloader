...
export const DEFAULT_AVIF_CONVERSION_WORKERS = 5;
...
export function sourceAvifEnabledKey(sourceName: string): string {
  return `source.${sourceName}.avif_enabled`;
}

export function sourceAvifQualityKey(sourceName: string): string {
  return `source.${sourceName}.avif_quality`;
}

export function getAvifConversionWorkers(settings: SettingsMap): number {
  return getPositiveIntegerSetting(
    settings.avif_conversion_workers,
...
export function getSourceAvifEnabled(settings: SettingsMap, sourceName: string): boolean {
  const key = sourceAvifEnabledKey(sourceName);
  return (settings[key] ?? "false") === "true";
}

export function getSourceAvifQuality(settings: SettingsMap, sourceName: string): number {
  const key = sourceAvifQualityKey(sourceName);
  const parsed = Number.parseInt(settings[key] ?? "80", 10);
...
export function buildSettingsSaveSnapshot(
  ...
  const payload: SettingsMap = {
    ...settingsSnapshot,
    avif_conversion_workers: `${getAvifConversionWorkers(settingsSnapshot)}`,
  ...
  for (const sourceName of sourceNames) {
    payload[sourceAvifEnabledKey(sourceName)] =
      settingsSnapshot[sourceAvifEnabledKey(sourceName)] ?? "false";
    payload[sourceAvifQualityKey(sourceName)] =
      settingsSnapshot[sourceAvifQualityKey(sourceName)] ?? "80";
  }
