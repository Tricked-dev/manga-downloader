<script lang="ts">
  ...
  const oidcConfigured = $derived(
    Boolean(
      ...
      settings.auth_oidc_client_secret?.trim(),
    ),
  );
...
<SettingsCard title={$t("app.settings.auth.title")} description={$t("app.settings.authDescription")}>
  ...
  <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_12rem]">
    ...
    <SettingField
      id="auth-provider-id"
      label={$t("app.settings.auth.providerId")}
      hint={$t("app.settings.auth.providerIdHint")}
    >
      <Input
        id="auth-provider-id"
        type="text"
        class="h-9"
        bind:value={settings.auth_oidc_provider_id}
        placeholder="oidc"
      />
    </SettingField>
  </div>
