...
export function hasPublicShareFlag(url: URL): boolean {
  return ["true", "1"].includes(url.searchParams.get(PUBLIC_SHARE_QUERY_PARAM) ?? "");
}
...
export function isPublicShareViewActive(
  ...
  return Boolean(
    ...
    auth.publicAccess &&
    (url.pathname === auth.publicAccess.pathname ||
      (url.pathname.startsWith("/read/") &&
        auth.publicAccess.pathname === `/library/${url.searchParams.get("libraryId")}`)),
  );
}
