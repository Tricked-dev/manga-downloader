...
export function createLibraryDetailState(
  ...
  let selectedUpscaleLoading = $state(false);
  ...
  const upscaleMutation = createUpscaleDownload();
  ...
  const upscaleActionDisabled = $derived(chapterActionAvailability.upscaleDisabled);
  ...
  function handleUpscaleSelected(event: MouseEvent) {
    runBulkAction(event, upscaleActionDisabled, upscaleSelectedDownloads);
  }
  ...
  async function upscaleSelectedDownloads() {
    if (!libraryId || selectedUpscaleLoading) {
      return;
    }

    const downloadsToUpscale = [...selectedCompletedDownloads];
    if (downloadsToUpscale.length === 0) {
      chapterActionMessageIsError = false;
      chapterActionMessage =
        selectedCount === 0
          ? translate("app.libraryDetail.selectDownloadedUpscale")
    ...
    chapterActionMessage = "";
    chapterActionMessageIsError = false;
    selectedUpscaleLoading = true;

    try {
      await upscaleLocalLibraryDownloads(upscaleMutation, downloadsToUpscale);
      await Promise.all([
        invalidateLocalLibraryDownloads(queryClient),
        invalidateLocalLibraryState(queryClient, libraryId),
      ]);
      chapterTable.resetRowSelection();
      chapterActionMessage =
        downloadsToUpscale.length === 1
          ? translate("app.libraryDetail.queueUpscaleOne")
          : translate("app.libraryDetail.queueUpscale", { count: downloadsToUpscale.length });
    } catch (error: unknown) {
      chapterActionMessageIsError = true;
      chapterActionMessage = errorMessage(error);
    } finally {
      selectedUpscaleLoading = false;
    }
  }
  ...
  return {
    ...
    get upscaleActionDisabled() {
      return upscaleActionDisabled;
    },
    refreshChapters,
    ...
    get selectedUpscaleLoading() {
      return selectedUpscaleLoading;
    },
