<script lang="ts">
  ...
  import { QUERY_CACHE_STORAGE_KEY, QUERY_CACHE_TIMES } from "$lib/query-client";
...
{#snippet accountControls()}
  {#if page.data.auth?.authenticated}
    <div class="border-t border-border/80 p-3">
      <Button variant="ghost" class="w-full justify-start gap-3" disabled={signingOut} onclick={signOut}>
        <LogOut class="size-5" />
        {$t("app.actions.signOut")}
      </Button>
      {#if signOutFailed}
        <p role="alert" class="px-3 text-xs text-destructive">{$t("app.login.signOutFailed")}</p>
      {/if}
    </div>
  {/if}
{/snippet}
