<script lang="ts">
  ...
  async function loadStatus(urlToCheck: string, signal?: AbortSignal) {
    ...
    try {
      ...
      status = readShareStatus(await response.json());
    } catch (caught) {
  ...
  function publicShareApiUrl(urlToCheck: string): string {
    const seriesId = new URL(urlToCheck).pathname.split("/")[2];
    return `/auth/public-share/${encodeURIComponent(seriesId)}`;
  }
  function readShareStatus(value: { share: { url: string } | null }): ShareStatus {
    return { public: Boolean(value.share), shareUrl: value.share?.url ?? null };
  }
</script>
