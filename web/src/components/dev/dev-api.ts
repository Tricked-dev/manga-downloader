...
export async function loadDevSession(
  ...
  const session = await response.json() as Awaited<ReturnType<typeof readSession>>;
  return { authEnabled: session.authEnabled, authenticated: Boolean(session.user), session: session.user ? { user: session.user } : null };
}
...
export async function loadDevAdminSummary(
  ...
  const [health, info, settings, library, downloads, sources] = await Promise.allSettled([
    fetchJson<HealthResponse>("/v1/health", signal),
    fetchJson<InfoResponse>("/v1/info", signal),
    fetchJson<SettingsResponse>("/v1/settings", signal),
    fetchJson<ListResponse<unknown>>("/v1/library", signal),
    fetchJson<ListResponse<DownloadRow>>("/v1/downloads", signal),
    fetchJson<ListResponse<SourceInfo>>("/v1/sources", signal),
  ]);
