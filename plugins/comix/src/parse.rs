...
use crate::{PluginResult, non_retryable_plugin_error as plugin_error};
...
pub fn parse_search_response(body: &str) -> PluginResult<SearchResponse> {
...
pub fn parse_manga_details_response(body: &str) -> PluginResult<SingleMangaResponse> {
    let normalized = extract_json_body(body);
...
pub fn parse_manga_details_page(
    body: &str,
    hash_id: &str,
) -> PluginResult<SingleMangaResponse> {
    let initial_data = extract_initial_data_json(body)?;
    let value = parse_json_value(initial_data, "manga details page", "parse_manga_details_page")?;
    let queries = value
        .get("queries")
...
pub fn parse_pages_response(body: &str) -> PluginResult<Vec<ParsedPage>> {
    let normalized = extract_json_body(body);
...
pub fn parse_genre_terms_response(body: &str) -> PluginResult<Vec<ApiTerm>> {
    let normalized = extract_json_body(body);
...
fn parse_json_value(body: &str, label: &str, code: &str) -> PluginResult<serde_json::Value> {
    serde_json::from_str(body)
        .map_err(|err| plugin_error(code, format!("Failed to parse {label}: {err}")))
}
...
fn extract_initial_data_json(body: &str) -> PluginResult<&str> {
